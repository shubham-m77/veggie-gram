...
const searchCategory = categories.find(
  ...
  const filteredProducts=products.filter((prd:any)=> prd.category.toLowerCase()===category);
  return (
    <div className="my-4 ">
      {searchCategory && (<div className='relative'><h2 className='text-2xl md:text-3xl font-medium'>{searchCategory.text.toUpperCase()}</h2>
      <hr className='border-1 border-primary-dull w-[70px] absolute top-9'/>
      </div>)}
      {filteredProducts.length>0?(<div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-5 items-center justify-center  gap-4 lg:gap-7 my-8'>
        {filteredProducts.map((product:any,idx:number) => (
         <div key={idx} onClick={() => navigate(`/veggies/${product.category.toLowerCase()}/${product._id}`)} className="border border-gray-500/20 gap-7 rounded-md md:px-4 px-3 py-2 bg-gray-50 w-full">
      ...
      </div>):(<div className="flex items-center justify-center my-4 text-2xl text-red-600">No Products Available!</div>)}
    </div>
  )
}
